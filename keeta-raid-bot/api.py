        ...
        try:
            ...
            payload = {
                "request": {
                    ...
                    "amount": "0xDE0B6B3A7640000",  # 1 token if 18 decimals
                    "affinity": "from"
                }
            }
            ...
            if not data.get("ok"):
                print("Murph estimate failed:", data)
                await asyncio.sleep(5)
                continue

            estimate = data.get("estimate", {})
            raw_amount_hex = estimate.get("convertedAmount")

            if not raw_amount_hex:
                print("Missing convertedAmount:", data)
                await asyncio.sleep(5)
                continue

            # Hex → integer base units → readable token amount
            raw_amount_int = int(raw_amount_hex, 16)

            # IMPORTANT:
            # Use 18 decimals only if the converted token uses 18 decimals.
            # If this token uses fewer decimals, change this value.
            converted_amount = raw_amount_int / (10 ** 18)
            ...
            murph_price_usd = converted_amount * keeta_usd_float
